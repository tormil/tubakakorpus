...
```{r proovin valja visata asju}
stop_tykid  <- c("konfisk", "sprott", "korter", "tuba", "köök", "üür", "tööd", "laps", "abielupaar", "kilu", "noormees", "teenistus")
stopp_regex <- paste(stop_tykid, collapse = "|")   # ei ole ^$, sest otsid fraasi teksti seest

paevalehe_reklaamid <- paevalehe_reklaamid %>% 
  filter(!str_detect(context, regex(stopp_regex, ignore_case = TRUE)))
#kell vb mitte
```
...
VAJUTA PÄRAST CONCS VÕRDLUST!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
```{r vaikiv ajastu ja varasem aeg TEST2 TOP2VEC} 
################################################################################
# 2. Tarvilikud teegid ---------------------------------------------------------
################################################################################
library(top2vecr)
...
library(tibble)   
################################################################################
# 3. Dokumendifailide ettevalmistus -------------------------------------------
#    top2vecr ootab data.frame'i veergudega doc_id + text                    --
################################################################################
docs_20_34_df <- paevalehe_reklaamid %>% 
  filter(LogicalSectionType == "ADVERTISEMENT",
         year >= 1920, year <= 1934) %>% 
  group_by(id) %>% 
  summarise(text = str_c(context, collapse = " "), .groups = "drop") %>% 
  transmute(doc_id = as.character(id), text)

docs_34_40_df <- paevalehe_reklaamid %>% 
  filter(LogicalSectionType == "ADVERTISEMENT",
         year >= 1934, year <= 1940) %>% 
  group_by(id) %>% 
  summarise(text = str_c(context, collapse = " "), .groups = "drop") %>% 
...
################################################################################
# 4. Mudelite treenimine top2vecr()--------------------------------------------
#    Võid timmida nt dim, n_neighbours, minPts jne (vaata ?top2vecr)          --
################################################################################
model_20_34 <- top2vecr(
  x           = docs_20_34_df,
  dim         = 300L,            # embedding‑dimensioon
  n_neighbours= 15L,
  n_components= 5L,
  minPts      = 30L
)

model_34_40 <- top2vecr(
  x           = docs_34_40_df,
  dim         = 300L,
  n_neighbours= 15L,
  n_components= 5L,
...
################################################################################
# 5. Teematopisõnade väljavõte -------------------------------------------------
#    top2vecr tagastab listi, kus iga element = data.frame 10 parima sõnaga   --
################################################################################
get_top_words <- function(topic_list) {
  map_dfr(topic_list, \(df) {
    tibble(
      topic = unique(df$topic),
      words = paste(df$word, collapse = ", ")
    )
  })
}

t20 <- get_top_words(model_20_34)
t34 <- get_top_words(model_34_40)

topics_wide <- full_join(
  t20 %>% rename(words_20_34 = words),
  t34 %>% rename(words_34_40 = words),
  by = "topic"
) %>% arrange(topic)

################################################################################
# 6. Tulemuse tabel (knitr või print) -----------------------------------------
################################################################################
knitr::kable(
  topics_wide,
  col.names = c("Teema #", "1920–34 (top sõnad)", "1934–40 (top sõnad)"),
  caption   = "top2vecr‑i 10 põhiteemat kahes ajavahemikus"
)
```
...
```{r bigrammid, ei lase knittida, eval=F}
bigrams <- suits %>%
